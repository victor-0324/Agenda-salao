...
def register():
    ...
    if request.method == "POST":
        ...
        if plan not in Config.PLANS:
            plan = "pro"
        theme = request.form.get("theme", "feminino")
        ...
        if errors:
            for e in errors:
                flash(e, "danger")
            return render_template("auth/register.html", selected_plan=plan, selected_theme=theme, form=request.form)
        ...
        login_user(user)
        flash(f"Conta criada! Você tem {Config.TRIAL_DAYS} dias de teste grátis. Seu link de agendamento é /agendar/{salon.slug}", "success")
        return redirect(url_for("dashboard.index"))

    return render_template("auth/register.html", selected_plan=selected_plan, selected_theme="feminino", form={})
