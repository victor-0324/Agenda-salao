...
class Salon(db.Model):
    ...
    # Versão visual do painel/página pública: "feminino" (rosa/roxo) ou "masculino" (azul/grafite)
    theme = db.Column(db.String(20), default="feminino", nullable=False)

    # Teste grátis: preenchido automaticamente no cadastro (created_at + TRIAL_DAYS)
    ...
    def trial_days_left(self):
        """Dias restantes de teste grátis (0 se já expirou ou não houver trial)."""
    ...
    def trial_total_days(self):
        if not self.trial_ends_at:
            return 0
        return max(1, (self.trial_ends_at - self.created_at).days)
    ...
    def trial_percent_left(self):
        ...
        if not total:
            return 0
        return round(min(100, max(0, (self.trial_days_left / total) * 100)))
    ...
    def is_booking_active(self):
        """Salão pode receber novos agendamentos pelo link público?"""
